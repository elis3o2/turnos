...
import type { Efector, Servicio, Especialidad, EfeSerEsp, EfeSerEspCompleto, SerEsp, Deriva } from "../efector/types";
...
export const getServiciosByEfector = (ids_efec: number[]): Promise<Servicio[]> => {
  const params = new URLSearchParams();
  ids_efec.forEach(id => {params.append("ids_efec", String(id))});
  return http.get<Servicio[]>(`efector/efe_ser_esp/servicios/?${params.toString()}`).then(res => res.data);
};
